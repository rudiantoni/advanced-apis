# v0.0.6b (Sub-Phase 6b): JWT security layer (basic, mock credentials)
...
3. Load the JWT secret, expiration, public-route rules, and mock super-user credentials from configuration (`application.yml` or environment).
...
- Login uses mock credentials from `app.security.super-user` (loaded into `AppProperties` at startup): email `admin@mail.com`, password `adminpass`. Wrong credentials return `401` with body `{"message":"Unauthorized"}`.
...
- `POST /users/create` and other user persistence endpoints are **not** part of this sub-phase (they belong to a later evolution with database-backed users).
...
Adds Spring Security and JWT (jjwt). Other dependencies are unchanged from previous phases.
...
Merge into the existing file from **previous phases** (`# ...` keeps the rest). Adds `app.security.*` and excludes the default in-memory user auto-configuration. `jwt-secret` must be a Base64-encoded key suitable for HS256.
...
Replaces const examples with JWT-related constants in the consts holder.
...
Replaces the placeholder property with JWT-related and super-user fields in the properties holder, along with getters and single-assignment setters.
...
Extends the `AppConfig`: `initializeJsonUtil()` is unchanged; `init()` also calls `initializeProperties()`.
