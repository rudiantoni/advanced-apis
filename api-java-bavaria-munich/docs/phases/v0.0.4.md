# v0.0.4 (Phase 4): PATCH update

Goals:

1. Distinguish a field that was not sent from a field sent as `null`
2. Add `PATCH /products/{id}` with update semantics and a dedicated `ProductPartialRequestDto`:
...
### `api-java-bavaria-munich/src/main/java/com/myapps/bavariamunich/dto/ProductPartialRequestDto.java`

Partial request DTO for updates (`PATCH /products/{id}`), without an `id` field, representing only the data the client sends, with distinction between a field that was sent and one sent as `null`.
